...
use App\Http\Requests\Api\V1\SalesInvoice\SalesInvoicePrintRequest;
...
class SalesInvoicePrintController extends JsonApiController
{
    /**
     ...
     * @return mixed
     */
    public function download(SalesInvoicePrintRequest $request, SalesInvoice $order)
    {
        $filename = Str::slug($order->code).'.pdf';
        /** @var DomPDFPDF */
        $pdf = Pdf::loadView(
            'pdf.salesInvoice.print',
            [
                'filename' => $filename,
                'order' => $order,
            ]
    ...
    public function sendMail(MailSendFormRequest $request, SalesInvoice $order)
    {
        ...
        $response = DocumentsMailSender::sendMailWithAttachment('pdf.salesInvoice.print',
            [
                'filename' => $filename,
                'order' => $order,
            ],
