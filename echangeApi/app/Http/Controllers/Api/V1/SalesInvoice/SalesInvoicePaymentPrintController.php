...
use App\Http\Requests\Api\V1\SalesPayment\SalesPaymentPrintRequest;
...
class SalesInvoicePaymentPrintController extends JsonApiController
{
    /**
     ...
     * @return mixed
     */
    public function download(SalesPaymentPrintRequest $request, SalesInvoice $order)
    {
        $filename = Str::slug($order->code).'.pdf';
        /** @var DomPDFPDF */
        $pdf = Pdf::loadView(
            'pdf.salesPayment.print',
            [
                'filename' => $filename,
                'order' => $order,
            ]
     ...
     * @return mixed
     */
    public function sendMail(MailSendFormRequest $request, SalesInvoice $order)
    {
        $data = $request->all();
        ...
        $response = DocumentsMailSender::sendMailWithAttachment('pdf.salesPayment.print',
            [
                'filename' => $filename,
                'order' => $order,
            ],
