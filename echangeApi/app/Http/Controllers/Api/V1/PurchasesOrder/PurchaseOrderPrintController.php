...
use App\Http\Requests\Api\V1\PurchasesOrder\PurchaseOrderPrintRequest;
...
class PurchaseOrderPrintController extends JsonApiController
{
    /**
     ...
     * @return mixed
     */
    public function download(PurchaseOrderPrintRequest $request, PurchasesOrder $order)
    {
        $filename = Str::slug($order->code).'.pdf';
        /** @var DomPDFPDF */
        $pdf = Pdf::loadView(
            'pdf.purchasesOrder.print',
            [
                'filename' => $filename,
                'order' => $order,
            ]
     ...
     * @return mixed
     */
    public function sendMail(MailSendFormRequest $request, PurchasesOrder $order)
    {
        $data = $request->all();
        ...
        $response = DocumentsMailSender::sendMailWithAttachment('pdf.purchasesOrder.print',
            [
                'filename' => $filename,
                'order' => $order,
            ],
