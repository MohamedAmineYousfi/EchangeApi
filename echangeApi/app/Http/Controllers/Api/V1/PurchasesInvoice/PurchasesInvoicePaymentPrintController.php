...
use App\Http\Requests\Api\V1\PurchasesInvoice\PurchaseInvoicePrintRequest;
...
class PurchasesInvoicePaymentPrintController extends JsonApiController
{
    /**
     ...
     * @return mixed
     */
    public function download(PurchaseInvoicePrintRequest $request, PurchasesInvoice $order)
    {
        $filename = Str::slug($order->code).'.pdf';
        /** @var DomPDFPDF */
        $pdf = Pdf::loadView(
            'pdf.purchasesPayment.print',
            [
                'filename' => $filename,
                'order' => $order,
            ]
     ...
     * @return mixed
     */
    public function sendMail(MailSendFormRequest $request, PurchasesInvoice $order)
    {
        $data = $request->all();
        ...
        $response = DocumentsMailSender::sendMailWithAttachment('pdf.purchasesPayment.print',
            [
                'filename' => $filename,
                'order' => $order,
            ],
