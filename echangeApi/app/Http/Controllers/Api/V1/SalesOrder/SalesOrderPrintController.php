...
use App\Http\Requests\Api\V1\SalesOrder\SalesOrderPrintRequest;
...
class SalesOrderPrintController extends JsonApiController
{
    /**
     ...
     * @return mixed
     */
    public function download(SalesOrderPrintRequest $request, SalesOrder $order)
    {
        $filename = Str::slug($order->code).'.pdf';
        /** @var DomPDFPDF */
        $pdf = Pdf::loadView(
            'pdf.salesOrder.print',
            [
                'filename' => $filename,
                'order' => $order,
            ]
     ...
     * @return mixed
     */
    public function sendMail(MailSendFormRequest $request, SalesOrder $order)
    {
        $data = $request->all();
        ...
        $response = DocumentsMailSender::sendMailWithAttachment('pdf.salesOrder.print',
            [
                'filename' => $filename,
                'order' => $order,
            ],
