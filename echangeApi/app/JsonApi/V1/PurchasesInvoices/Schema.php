<?php

namespace App\JsonApi\V1\PurchasesInvoices;

use App\Models\PurchasesInvoice;
use Neomerx\JsonApi\Schema\SchemaProvider;

class Schema extends SchemaProvider
{
    /**
     * @var string
     */
    protected $resourceType = 'purchases-invoices';

    /**
     * @param  PurchasesInvoice  $resource
     *                                      the domain record being serialized.
     * @return string
     */
    public function getId($resource)
    {
        return (string) $resource->getRouteKey();
    }

    /**
     * @param  PurchasesInvoice  $resource
     *                                      the domain record being serialized.
     * @return array
     */
    public function getAttributes($resource)
    {
        return [
            'excerpt' => $resource->excerpt ?? '',
            'code' => $resource->code,
            'expiration_time' => optional($resource->expiration_time)->format('Y-m-d H:i:s'),
            'status' => $resource->status,
            'has_no_taxes' => (bool) $resource->has_no_taxes,
            'pricing' => [
                'taxes' => $resource->getInvoiceTaxes(),
                'discounts' => $resource->getInvoiceDiscounts(),
                'subtotal' => $resource->getInvoiceSubTotalAmount(),
                'total' => $resource->getInvoiceTotalAmount(),
            ],
            'total_paied' => $resource->getInvoiceTotalPaied(),
            'total_remaining_payment' => $resource->getInvoiceTotalAmount() - $resource->getInvoiceTotalPaied(),
            'created_at' => $resource->created_at,
            'updated_at' => $resource->updated_at,
        ];
    }

    public function getRelationships($item, $isPrimary, array $includeRelationships)
    {
        return [
            'items' => [
                self::SHOW_SELF => false,
                self::SHOW_RELATED => true,
                self::SHOW_DATA => isset($includeRelationships['items']),
                self::DATA => function () use ($item) {
                    return $item->items;
                },
            ],
            'issuer' => [
                self::SHOW_SELF => false,
                self::SHOW_RELATED => true,
                self::SHOW_DATA => true,
                self::DATA => function () use ($item) {
                    return $item->issuer;
                },
            ],
            'organization' => [
                self::SHOW_SELF => false,
                self::SHOW_RELATED => true,
                self::SHOW_DATA => isset($includeRelationships['organization']),
                self::DATA => function () use ($item) {
                    return $item->organization;
                },
            ],
            'purchasesOrder' => [
                self::SHOW_SELF => false,
                self::SHOW_RELATED => true,
                self::SHOW_DATA => isset($includeRelationships['purchasesOrder']),
                self::DATA => function () use ($item) {
                    return $item->purchasesOrder;
                },
            ],
            'allowedLocations' => [
                self::SHOW_SELF => false,
                self::SHOW_RELATED => true,
                self::SHOW_DATA => isset($includeRelationships['allowedLocations']),
                self::DATA => function () use ($item) {
                    return $item->allowedLocations ? $item->allowedLocations : [];
                },
            ],
            'destinationWarehouse' => [
                self::SHOW_SELF => false,
                self::SHOW_RELATED => true,
                self::SHOW_DATA => isset($includeRelationships['destinationWarehouse']),
                self::DATA => function () use ($item) {
                    return $item->destinationWarehouse;
                },
            ],
        ];
    }
}
