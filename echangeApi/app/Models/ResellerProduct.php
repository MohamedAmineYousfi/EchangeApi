 ...
 */
class ResellerProduct extends Model implements EventNotifiableContract, OnDeleteRelationsCheckable, ResellerInvoiceable, ResellerScopable
{
    use EventNotifiable {
    ...
    protected static function booted()
    {
        ...
        static::saving(function (ResellerProduct $product) {
            if (! $product->code) {
                $invoicesCount = ResellerProduct::withoutGlobalScopes()->count() + 1;
                $product->code = 'R-PRD-'.Carbon::now()->format('Ymd').str_pad(strval($invoicesCount), 6, '0', STR_PAD_LEFT);
            }
        });
    }
    ...
    {
        return $query->where('reseller_products.name', 'LIKE', "%$search%", 'or')
            ->where('reseller_products.code', 'LIKE', "%$search%", 'or');
    }
